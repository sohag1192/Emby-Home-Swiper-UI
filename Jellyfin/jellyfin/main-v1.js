class CommonUtils {
    static selectWait(selector, func, times, interval) {
        var _times = times || 100, // 100 times
            _interval = interval || 500, // 20ms every time
            _jquery = null,
            _iIntervalID;
...
class HomeBanner {
    ...
    static async initBanner() {
        ...
        data.Items.forEach(async (item) => {
            ...
            itemHtml += `
					<div>
                        <p onclick="window.Emby.Page.showItem('${detail.Id}')"><strong>${detail.Name}</strong>${detail.Overview}</p>
                    </div>
					
					<div>
                        <button id="${detail.Id}" data-parentid="${detail.Id}" class="banner-item-play" tabindex="-1">PLAY</button>
    ...
    static initEvent() {
        ...
        // Added event listener for Play button since it was missing logic
        $(document).on('click', '.banner-item-play', function() {
            var id = $(this).attr('id');
            if(window.Emby && window.Emby.Page) {
               // Logic to play, usually requires more specific Emby/Jellyfin calls
               // For now defaulting to opening the item similar to showItem
