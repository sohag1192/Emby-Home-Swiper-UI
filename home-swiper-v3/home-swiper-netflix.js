/**
 * ====================================================================
 * Emby / Jellyfin Netflix Billboard Hero Slider (Version 3.0)
 * Fully bundled standalone script with Swiper 11.1.14 and Netflix UI
 * Author: Md. Sohag Rana (sohag1192)
 * Repository: https://github.com/sohag1192/Emby-Home-Swiper-UI
 * ====================================================================
 */
...
class HomeSwiper {
	static start() {
		this.itemQuery = { 
			...
			ImageTypeLimit: 1, 
			Limit: 12, 
			Fields: "Taglines,Overview,ParentId,DateCreated,PremiereDate,ProductionYear,CommunityRating,OfficialRating,RunTimeTicks,Genres", 
		...
		this.logoOptions = { type: "Logo", maxWidth: 600, adjustForPixelRatio: false };
		this.coverOptions = { type: "Primary", maxWidth: 1000, adjustForPixelRatio: false };
		this.showItemNum = 12;
		this.slideDelay = 7000;
		this.cacheVersion = "v3-netflix-edition-v1";
		this.loadFlag = false; 
		...
		function saveUser(instance, page) {
			...
			user.Policy.EnabledFolders = user.Policy.EnableAllFolders
				...
				: Array.prototype.filter.call(page.querySelectorAll(".chkFolder"), function (c) {
					return c.checked;
				}).map(function (c) {
			...
			require(["toast"], function (toast) {
				toast("Netflix Swiper Settings Saved");
			});
		}

		function renderMediaFolders(page, user, mediaFolders) {
			var html = '<div class="checkboxListLabel"><label><h3 style="margin: 0 10px 0 0;"> Select all media libraries</h3><input type="checkbox" is="emby-checkbox" class="chkAllFolder"><span style="font-size: .7em;"><h3 style="margin:0;"></h3></span></label></div><div class="checkboxLists">';
			for (var i = 0, length = mediaFolders.length; i < length; i++) {
				var folder = mediaFolders[i],
					folderId = folder.Guid || folder.Id,
					isChecked = (user.Policy.EnabledFolders.length === 0 && user.Policy.EnableAllFolders) || user.Policy.EnabledFolders.includes(folderId);
				html += '<div class="checkboxList" style="margin:0 0 1.5em;"><label><input type="checkbox" is="emby-checkbox" class="chkFolder" data-id="' + folderId + '" ' + (isChecked ? ' checked="checked"' : "") + '><span><h3 style="margin:0;">' + folder.Name + "</h3></span></label></div>";
			}
			html += "</div>";
			page.querySelector(".folderAccess").innerHTML = html;
			var chkAll = page.querySelector(".chkEnableAllFolders");
			if (chkAll) chkAll.checked = user.Policy.EnableAllFolders;
		}
		...
		document.addEventListener("viewbeforeshow", async function (e) {
			...
			if (e.detail.type === "home") {
				if (!e.detail.isRestored) {
					...
					!document.getElementById("netflixCss") && CommonUtils.loadExtastyle(this.netflixCss, 'netflixCss');
					...
					!localStorage.getItem('CACHE|Netflix-Date') && !this.loadFlag && this.initLoading();
					e.target.setAttribute("data-type", "home");
					this.mutation = new MutationObserver(async function (mutationRecords) {
						for (let mutationRecord of mutationRecords) {
							if (mutationRecord.target.classList.contains("homeSectionsContainer")) {
								...
								if (elem) {
									...
									this.loadFlag && this.fadeOut(document.querySelector(".misty-loading"), 500, () => document.querySelector(".misty-loading")?.remove());
									document.getElementById("netflixCss")?.remove();
					...
					this.swiperMain = e.target.querySelector('.nflx-hero-swiper')?.swiper;
					this.swiperMain?.enable();
					this.swiperMain?.autoplay?.start();
				}
			} else {
				this.mutation_cssjs?.disconnect();
				this.mutation?.disconnect();
				this.swiperMain?.disable();
					...
					if (!e.detail.isRestored) {
						...
						<div class="verticalSection">
						   <div class="folderAccessContainer">
						      <h2>Netflix Billboard Carousel Access</h2>
						      ...
						      <div class="folderAccessListContainer">
						         ...
						         <div class="folderAccess"></div>
						      </div>
						      <div class="fieldDescription">Select the media libraries to display in the Netflix billboard slider.</div>
						   </div>
						   <br />
						   <div>
						...
						view.querySelector(".userLibraryAccessForm")?.addEventListener("click", function (evt) {
							saveUser(this, view);
							evt.preventDefault();
							evt.stopPropagation();
							return false;
		...
		this.mutation_cssjs = new MutationObserver(async function (mutationRecords) {
			if (!this.flag_cssjs) {
				this.mutation_cssjs.disconnect();
				return;
			}
			...
			if (view?.controller?.constructor.name === "HomeView") {
				for (let mutationRecord of mutationRecords) {
					if (mutationRecord.target.classList.contains("homeSectionsContainer")) {
						...
						!document.getElementById("netflixCss") && CommonUtils.loadExtastyle(this.netflixCss, 'netflixCss');
						!localStorage.getItem('CACHE|Netflix-Date') && !this.loadFlag && this.initLoading();
						let sections = mutationRecord.target.querySelector(".sections"),
							elem = await this.initBanner();
						if (elem) {
							sections.parentNode.insertBefore(elem, sections);
							this.loadFlag && this.fadeOut(document.querySelector(".misty-loading"), 500, () => document.querySelector(".misty-loading")?.remove());
	...
	static async playlistener() {
		...
		document.querySelectorAll(".view:not(.hide) .banner-item-play").forEach(async (dom) => {
			let id = dom.getAttribute("id");
			let parentid = dom.getAttribute("data-parentid");
			const detail = this.flatItems?.find(m => m.Id === id);
			if (!detail) return;
			dom.onclick = (e) => {
				e.stopPropagation();
				let resume = {
					startPositionTicks: detail.UserData?.PlaybackPositionTicks || 0,
					ids: detail.Id,
	...
	static morelistener() {
		document.querySelectorAll(".view:not(.hide) .banner-item-more, .view:not(.hide) .nflx-clickable").forEach(dom => {
			dom.onclick = (e) => {
				e.stopPropagation();
				const id = dom.getAttribute("data-id") || dom.getAttribute("id");
	...
	static async getLibItems(refreshFlag) {
		...
		if (refreshFlag || !localStorage.getItem(cacheKey)) {
			...
			for (let i = 0; i < libdataitem.length; ++i) {
				...
				for (let item of dataquery.Items) {
					if (!flatList.some(x => x.Id === item.Id)) {
						item._libraryName = libitem.Name;
						flatList.push(item);
					}
				}
			}
			flatList.sort((a, b) => this.getNewestItemTime([b]) - this.getNewestItemTime([a]));
			if (flatList.length > this.showItemNum) {
				flatList = flatList.slice(0, this.showItemNum);
			}
			const data = JSON.stringify(flatList);
			localStorage.setItem(cacheKey, data);
			require(["datetime"], function (datetime) {
				localStorage.setItem('CACHE|Netflix-Date', datetime.toLocaleDateString(new Date(), { dateStyle: "short" }));
			});
		} else {
			require(["datetime"], function (datetime) {
				if (localStorage.getItem('CACHE|Netflix-Date') !== datetime.toLocaleDateString(new Date(), { dateStyle: "short" })) {
	...
	static getImageUrl(item, options) {
		...
		return options.type == "Logo" && imageTags && imageTags.Logo ? (imgUrl = ApiClient.getImageUrl(item.Id, {
			...
			type: "Backdrop", maxWidth: width, tag: item.BackdropImageTags[0], adjustForPixelRatio: adjustForPixelRatio
		})) : (imageTags && imageTags.Primary ? ApiClient.getImageUrl(item.Id, {
			type: "Primary", maxWidth: width, tag: imageTags.Primary, adjustForPixelRatio: adjustForPixelRatio
		}) : "");
	}
	...
	static fadeOut(elemt, speed, callback) {
		if (elemt && (elemt.style.opacity == 1 || elemt.style.opacity == "")) {
	...
	static async initBanner() {
		...
		element.className = "verticalSection verticalSection-cards section00 nflx-hero-section focusable";
		element.setAttribute("data-focusabletype", "nearest");
		element.innerHTML = `
		<div class="swiper nflx-hero-swiper">
		   <div class="swiper-wrapper"></div>
		   <div class="nflx-pagination"></div>
		   <div class="nflx-nav-btn nflx-nav-prev" title="Previous Slide">
		      <svg viewBox="0 0 24 24" width="28" height="28" fill="currentColor"><path d="M15.41 7.41L14 6l-6 6 6 6 1.41-1.41L10.83 12z"/></svg>
		   </div>
		   <div class="nflx-nav-btn nflx-nav-next" title="Next Slide">
		      <svg viewBox="0 0 24 24" width="28" height="28" fill="currentColor"><path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"/></svg>
		   </div>
		</div>`;

		this.flatItems = await this.getLibItems(false);
		if (!this.flatItems || this.flatItems.length === 0) {
			return null;
		}

		let slidesHtml = "";
		for (let j = 0; j < this.flatItems.length; ++j) {
			let detail = this.flatItems[j];
			let ImageUrl = await this.getImageUrl(detail, this.backdropOptions);
			if (!ImageUrl) {
			...
			const mediaTypeLabel = isSeries ? "SERIES" : "FILM";
			const logoUrl = detail.ImageTags?.Logo ? await this.getImageUrl(detail, this.logoOptions) : "";
			const year = detail.ProductionYear ? this.escapeHtml(detail.ProductionYear) : (detail.PremiereDate ? new Date(detail.PremiereDate).getFullYear() : "");
			
			// Match percentage calculation
			let matchPercent = "98% Match";
			if (detail.CommunityRating) {
				let calc = Math.min(99, Math.max(78, Math.round(Number(detail.CommunityRating) * 10)));
				matchPercent = `${calc}% Match`;
			}
			
			const officialRating = this.escapeHtml(detail.OfficialRating || (isSeries ? "TV-MA" : "PG-13"));
			const rankNumber = j + 1;

			const slide = `
			<div class="swiper-slide nflx-hero-slide" id="${detail.Id}">
			   <div class="nflx-backdrop-container">
			      <img id="${detail.Id}" class="nflx-backdrop-img banner-cover" draggable="false" loading="${j === 0 ? 'eager' : 'lazy'}" decoding="async" src="${ImageUrl}" alt="${title}" />
			   </div>
			   <div class="nflx-gradient-overlay"></div>
			   
			   <div class="nflx-content-box">
			      <div class="nflx-badge-row" data-swiper-parallax="-350">
			         <span class="nflx-n-logo">N</span>
			         <span class="nflx-media-type">${mediaTypeLabel}</span>
			      </div>

			      <div class="nflx-title-logo-container nflx-clickable" data-id="${detail.Id}" data-swiper-parallax="-300">
			         ${logoUrl ? `<img draggable="false" loading="${j === 0 ? 'eager' : 'lazy'}" decoding="async" class="nflx-title-logo" src="${logoUrl}" alt="${title}">` : `<h1 class="nflx-title-text">${title}</h1>`}
			      </div>

			      <div class="nflx-top10-row" data-swiper-parallax="-250">
			         <div class="nflx-top10-badge">TOP<br>10</div>
			         <span class="nflx-top10-text">#${rankNumber} in ${detail._libraryName || (isSeries ? 'TV Shows' : 'Movies')} Today</span>
			      </div>

			      <div class="nflx-meta-row" data-swiper-parallax="-200">
			         <span class="nflx-match">${matchPercent}</span>
			         ${year ? `<span class="nflx-year">${year}</span>` : ''}
			         <span class="nflx-cert-pill">${officialRating}</span>
			         <span class="nflx-quality-pill">4K Ultra HD</span>
			         <span class="nflx-quality-pill">5.1</span>
			      </div>

			      <p class="nflx-synopsis" data-swiper-parallax="-150">${overview || subtitle}</p>

			      <div class="nflx-btn-row" data-swiper-parallax="-100">
			         <button id="${detail.Id}" data-parentid="${detail.ParentId || ''}" class="nflx-btn nflx-btn-play banner-item-play" tabindex="0">
			            <svg viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
			            <span>Play</span>
			         </button>
			         <button id="${detail.Id}" class="nflx-btn nflx-btn-info banner-item-more" tabindex="0">
			   ...
			   <div class="nflx-right-controls">
			      <div class="nflx-age-badge">${officialRating}</div>
			   </div>
			</div>`;
			slidesHtml += slide;
		}

		element.querySelector(".nflx-hero-swiper .swiper-wrapper").innerHTML = slidesHtml;
		return element;
	}

	static initSwiper(container) {
		const swiperEl = container.querySelector(".nflx-hero-swiper");
		if (!swiperEl) return;

		this.swiperMain = new Swiper(swiperEl, {
			loop: true,
			speed: 900,
			parallax: true,
			grabCursor: true,
			...
			pagination: {
				el: swiperEl.querySelector(".nflx-pagination"),
				clickable: true,
			},
			navigation: {
				nextEl: swiperEl.querySelector(".nflx-nav-next"),
				prevEl: swiperEl.querySelector(".nflx-nav-prev"),
			},
			keyboard: {
				enabled: true,
				onlyInViewport: true,
...
HomeSwiper.SwiperCss = `@font-face {
...
.banner-meta {
	...
	font-size: clamp(0.82rem, 0.44rem + 0.82vw, 1.08rem);
	font-weight: 800;
	line-height: 1.1;
...
HomeSwiper.netflixCss = `﻿/* =======================================================
   🍿 Netflix Billboard Theme for Emby & Jellyfin
   Author: Md. Sohag Rana (sohag1192)
   Version: 3.0 (Netflix Edition)
   ======================================================= */

:root {
  --nflx-red: #e50914;
  --nflx-red-hover: #f40612;
  --nflx-black: #141414;
  --nflx-dark-gray: #181818;
  --nflx-gray: #2f2f2f;
  --nflx-light-gray: #6d6d6e;
  --nflx-green: #46d369;
  --nflx-white: #ffffff;
}

/* Hero Section Container */
.section00.nflx-hero-section {
  width: 100%;
...
.swiper-slide-active .nflx-backdrop-img {
  transform: scale(1.06);
}

/* Netflix Vignettes & Gradients */
.nflx-gradient-overlay {
  position: absolute;
  top: 0;
  left: 0;
  width: 100%;
  ...
  pointer-events: none;
  background: 
    linear-gradient(77deg, rgba(0, 0, 0, 0.88) 0%, rgba(0, 0, 0, 0.6) 38%, rgba(0, 0, 0, 0) 75%),
    linear-gradient(180deg, rgba(20, 20, 20, 0) 0%, rgba(20, 20, 20, 0.3) 45%, rgba(20, 20, 20, 0.85) 80%, var(--nflx-black) 100%),
    linear-gradient(0deg, rgba(0, 0, 0, 0.4) 0%, transparent 25%);
}

/* Netflix Content Overlay Box */
.nflx-content-box {
  position: relative;
  z-index: 10;
  width: 100%;
  max-width: clamp(340px, 46vw, 700px);
...
/* Metadata Row (Match %, Year, Rating, Badges) */
.nflx-meta-row {
  display: flex;
  align-items: center;
  flex-wrap: wrap;
...
.nflx-match {
  color: var(--nflx-green);
  font-weight: 700;
}

.nflx-year, .nflx-duration {
  color: #bcbcbc;
}
...
/* Right Side Age Rating & Audio Controls */
.nflx-right-controls {
  position: absolute;
  right: 0;
  bottom: 25%;
...
/* Netflix Pagination Bars (Segmented Indicator) */
.nflx-pagination {
  position: absolute;
  top: 15px;
...
.nflx-pagination .swiper-pagination-bullet {
  width: 24px;
  height: 3px;
  border-radius: 1px;
...
.nflx-pagination .swiper-pagination-bullet-active {
  background: var(--nflx-red);
  width: 36px;
}
...
.nflx-nav-prev { left: 10px; }
.nflx-nav-next { right: 10px; }
...
// Initialize Netflix Hero Billboard Slider
HomeSwiper.start();
